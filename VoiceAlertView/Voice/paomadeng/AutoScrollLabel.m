...
- (void)animationDidStop:(NSString *)animationID finished:(NSNumber *)finished context:(void *)context
{
	...
	if ([finished intValue] == 1 && label[0].frame.size.width > self.frame.size.width){
		[NSTimer scheduledTimerWithTimeInterval:pauseInterval+1 target:self selector:@selector(scroll) userInfo:nil repeats:NO];
	}
} 
#endif
...
- (void) readjustLabels
{
	...
	for (int i = 0; i < NUM_LABELS; ++i){
		[label[i] sizeToFit];
        label[i].width = label[i].width > self.width ? label[i].width : self.width;
